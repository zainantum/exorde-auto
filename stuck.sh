...
do
    ...
    elif (tail -n45 log.txt | grep 'No new work' | wc -l) > "10" || tail -n5 log.txt | grep 'python: not found' || tail -n5 log.txt | grep 'sub routine initialized' || tail -n20 log.txt | grep 'Initialization error Something went' || tail -n20 log.txt | grep 'Read timed out'
    then
